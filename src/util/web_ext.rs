...
pub fn validate_uint_only() -> Callback<KeyboardEvent> {
	Callback::from(|evt: KeyboardEvent| {
		if !evt.cancelable() {
			log::error!("Cannot cancel keyboard event");
			return;
		}
...
pub trait CallbackOptExt<I: 'static, O: 'static> {
	...
	fn then_emit<O2: 'static>(self, next: Callback<O, O2>) -> Callback<I, Option<O2>>;
}
	...
	O: 'static,
{
	...
	fn then_emit<O2: 'static>(self, next: Callback<O, O2>) -> Callback<I, Option<O2>> {
		next.filter_reform(move |input: I| self.emit(input))
	}
}
